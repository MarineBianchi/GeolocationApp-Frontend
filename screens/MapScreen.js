...
import { addPlace, updateNickname, bddPlaces } from '../reducers/user';
...
export default function MapScreen() {
  ...
  const pla = useSelector((state) => state.user.value.places);
...
// fetch de l'afficahe des lieux en fonction de l'utilisateur
  useEffect(() => {
    (async () => {
    ...
    fetch(`http://10.20.2.178:3000/places/${user.nickname}`)
    .then((response) => response.json())
    .then((data) => {
     { data.result && dispatch(bddPlaces(data.places));
      }
     });
  }, []);

  // Récupération des données de la latitude et longitude en appuyant sur l'écran
  ...
  // fetch de la route d'ajout d'un nouveau lieu en appuyant sur l'écran
      ...
      <Modal visible={modalVisible} animationType="fade" transparent>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <TouchableOpacity onPress={() => handleNewPlace()} 
